...
def run_eda(data_path, output_path):
    ...
    # 缺失值移除
    rows_before_dropna = len(dedup_df)
    dedup_df = dedup_df.dropna()
    rows_after_dropna = len(dedup_df)
    na_removed = rows_before_dropna - rows_after_dropna

    # 异常值判断逻辑（这里没有实际移除，只做统计）
    age_removed = dedup_df[~dedup_df['age'].between(0, 100)].shape[0]
    income_removed = dedup_df[dedup_df['income'] < 0].shape[0]
    credit_removed = dedup_df[~dedup_df['credit_score'].between(0, 900)].shape[0]
    ...
    credit_rate = round(credit_removed / final_total * 100, 2)

    stats_text = f'''
移除重复记录: {dup_removed} （占原始样本 {dup_rate}%）
总移除缺失行: {na_removed} （占去重后数据 {na_rate}%）

各字段异常值移除:
- age: {age_removed} 条，占比 {age_rate}%
- income: {income_removed} 条，占比 {income_rate}%
- credit_score: {credit_removed} 条，占比 {credit_rate}%
...
{dedup_df["purchase_history"].apply(lambda x: json.loads(x).get("category", "未知")).value_counts()}
'''
